"use strict";(()=>{function c(e,a,i){switch(a){case"==":return e==i;case"===":return e===i;case"!=":return e!=i;case"!==":return e!==i;case"<":return e<i;case"<=":return e<=i;case">":return e>i;case">=":return e>=i;case"&&":return e&&i;case"||":return e||i;case"in":return typeof e=="string"&&typeof i=="string"?i.split(",").map(t=>t.trim()).includes(e):!1;case"contains":return typeof e=="string"&&typeof i=="string"?e.toLowerCase().indexOf(i.toLowerCase())>=0:!1;default:return!1}}function r(e){e.registerHelper("contains",function(a,i,t){return a=e.escapeExpression(a),i=e.escapeExpression(i),i.indexOf(a)>-1?t.fn(this):t.inverse(this)}),e.registerHelper("ifCond",function(a,i,t,s){return c(a,i,t)?s.fn(this):s.inverse(this)}),e.registerHelper("cond",(a,i,t)=>c(a,i,t)),e.registerHelper("isType",function(a,i,t){return a===i?t.fn(this):t.inverse(this)}),e.registerHelper("ifAny",function(...a){let i=a.pop();return a.some(t=>!!t)?i.fn(this):i.inverse(this)}),e.registerHelper("now",function(){return new Date().toISOString()}),e.registerHelper("formatDate",function(a,i,t){let s;if(a&&(s=new Date(a)),isNaN(s)&&i&&(s=new Date(i)),isNaN(s))return"Invalid Date";var l=["January","February","March","April","May","June","July","August","September","October","November","December"],o=s.getDate(),n=l[s.getMonth()],C=s.getFullYear();switch(t){case"YYYY":return`${C}`;case"MMMM YYYY":return`${n} ${C}`;default:return`${o} ${n} ${C}`}}),e.registerHelper("formatDateOrToday",function(a,i){let t=a||new Date().toISOString();return e.helpers.formatDate(a,t,i)}),e.registerHelper("formatDuration",function(a,i){if(!a)return"";if(typeof a=="string"&&i!=="long")return a;let t="",s=[],l,o,n;if(typeof a=="string"){let C=a.split(":");n=C[0],C.length==2?[o="",n=""]=C:C.length==3&&([l="",o="",n=""]=C)}else[l="",o="",n=""]=a;return i==="long"?(l>0&&s.push(`${l} hour${l>1?"s":""}`),o>0&&s.push(`${o} minute${o>1?"s":""}`),n>0&&s.push(`${n} second${n>1?"s":""}`),t=s.join(" ")):(l>0&&s.push(l.toString().padStart(2,0)),s.push(o.toString().padStart(2,0)),s.push(n.toString().padStart(2,0)),t=s.join(":")),t}),e.registerHelper("isdefined",function(a,i){return a!==void 0&&a!==""?a:i}),e.registerHelper("getClassNames",function(a,i){let t=a.split(",").map(l=>l.trim()),s=[];for(let l=0;l<t.length;l++){let o=t[l];if(i){for(let n=0;n<i.length;n++)if(i[n][o]===!0){s.push(o);break}}}return s.length>0?s.join(" "):""}),e.registerHelper("join",function(a,i){return(!i||typeof i!="string")&&(i=" "),Array.isArray(a)?a.join(i):a}),e.registerHelper("toCamelCase",function(a){return typeof a!="string"?a:a.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w|\s+)/g,(i,t)=>t===0?i.toLowerCase():i.toUpperCase()).replace(/\s+/g,"")})}typeof Handlebars!="undefined"&&r(Handlebars);var p=`<!-- QGDS Component: Accordion -->
...
</html>`;var y=`  <div class="row {{classes}}">
    {{#if lastUpdated}}
    <div class="col-auto">
...
<!-- VERSION_DETAILS={"project_id":"@qld-gov-au/qgds-bootstrap5","version":"2.0.7","branch":"QGDS-266-design-qa-header","tag":"","commit":"b1ef181cf9a131f60ad0fec3e6fd2477f0ee7452","majorVersion":"v2"} -->
...
</html>`;var F=`<svg class="qld-icon-{{size}}"{{#if ariaLabel}} aria-label="{{ariaLabel}}"{{/if}}{{#if ariaHidden}} aria-hidden="{{ariaHidden}}"{{/if}}>
  <use href="./assets/img/icons-sprite.svg#qgds-icon-{{iconId}}"></use>
</svg>`;var B=`{{! Image template }}
{{#*inline "imageTag"}}
...
{{#if src}}
  ...
  {{else}}
    {{> imageTag}}
  {{/if}}
{{/if}}`;var Q=`<div class="alert {{variantClass}} {{customClass}}" role="alert" 
...
{{> footer footer}}`;var X=`<!-- DCTERMS https://www.dublincore.org/specifications/dublin-core/dcmi-terms/-->
{{#if dcTerms}}
...
{{#if og.type }}<meta property="og:type" content="{{og.type}}" />{{/if}}`;var e5=`<!-- Example button to trigger the modal -->
...
</div>`;var i5=`<!-- QGDS Partial: Primary navigation -->
...
</nav>`;var t5=`<section class="qld-promo-panel {{#ifCond type '==' 'indent-text'}}indent{{/ifCond}} {{#ifCond type '==' 'indent-img'}}indent{{/ifCond}} {{type}}">
...
</section>`;var s5=`<!-- QGDS Component: Loading Quick exit -->
...
</div>`;var n5=`<!-- QGDS Component: Select -->
...
`;var o5=`<nav class="qld-side-navigation" aria-label="Side Navigation">
  <button
...
`;var r5=`<nav class="qld-skip-links" aria-label="skip to content links">
  {{#each skipLinks}}
    <a class="qld-skip-links__item" href="#{{targetId}}">{{label}}</a>
  {{/each}}
</nav>`;var d5=`<!-- QGDS Component: Loading Spinner -->
...
</section>`;var f5=`<!-- QGDS Component: Tag -->
...
`;var u5=`<!-- QGDS Component: Textarea -->
...
<textarea id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    aria-required="true" {{/if}}></textarea>`;var m5=`<!-- QGDS Component: Textbox -->
...
<input id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    isRequired}}required aria-required="true" {{/if}} />`;var g5="<!-- QGDS Partial: typography -->";var v5=`<!-- QGDS Component: Video -->
...
`;function d(e){e.registerPartial("accordion",p),e.registerPartial("backToTop",f),e.registerPartial("banner",u),e.registerPartial("blockquote",m),e.registerPartial("breadcrumbs",g),e.registerPartial("breadcrumbsWrapper",v),e.registerPartial("button",h),e.registerPartial("callout",b),e.registerPartial("callToAction",L),e.registerPartial("card",k),e.registerPartial("containerLayout",q),e.registerPartial("contentFooter",y),e.registerPartial("contentFooterWrapper",M),e.registerPartial("contentPageWithForm",V),e.registerPartial("contentPageWithSideNavigation",x),e.registerPartial("contentWrapper",w),e.registerPartial("correctincorrect",H),e.registerPartial("customLinks",Z),e.registerPartial("dateinput",T),e.registerPartial("directionLinks",S),e.registerPartial("feedbackForm",A),e.registerPartial("followLinks",P),e.registerPartial("footer",O),e.registerPartial("formcheck",R),e.registerPartial("fullPage",D),e.registerPartial("globalAlert",I),e.registerPartial("head",_),e.registerPartial("header",E),e.registerPartial("headerBrand",N),e.registerPartial("homePage",G),e.registerPartial("icon",F),e.registerPartial("image",B),e.registerPartial("inpageAlert",Q),e.registerPartial("inpagenav",j),e.registerPartial("link",J),e.registerPartial("linkColumns",W),e.registerPartial("logo",Y),e.registerPartial("logoCOALandscape",U),e.registerPartial("logoCOALandscape2Lines",$),e.registerPartial("mainContainerWrapper",z),e.registerPartial("metaDcTerms",X),e.registerPartial("metaOpenGraph",K),e.registerPartial("modal",e5),e.registerPartial("navbar",i5),e.registerPartial("pagination",a5),e.registerPartial("promotionalPanel",t5),e.registerPartial("quickexit",s5),e.registerPartial("searchInput",l5),e.registerPartial("select",n5),e.registerPartial("sidenav",o5),e.registerPartial("sidenavWrapper",C5),e.registerPartial("skipLinks",r5),e.registerPartial("spinner",d5),e.registerPartial("table",c5),e.registerPartial("tabs",p5),e.registerPartial("tag",f5),e.registerPartial("textarea",u5),e.registerPartial("textbox",m5),e.registerPartial("typography",g5),e.registerPartial("video",v5)}typeof Handlebars!="undefined"&&d(Handlebars);var h5=!1;function b5(e){typeof e=="undefined"&&console.error("Handlebars.init requires HandleBars"),typeof e!="undefined"?(h5?console.log("HandleBars Helpers And Partials already loaded, loading again"):h5=!0,r(e),d(e)):console.log("Handlebars not found, init failed")}typeof Handlebars!="undefined"&&b5(Handlebars);})();
