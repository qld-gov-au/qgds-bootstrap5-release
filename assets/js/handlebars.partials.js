...
<div class="accordion-group">
    ...
    <div class="accordion" id="{{groupid}}">
        ...
        <div class="accordion-item">
            ...
            <div id="collapse-{{id}}" class="accordion-collapse collapse {{#if expanded}}show{{/if}}" aria-labelledby="heading-{{id}}" role="region"{{#unless expanded}} hidden="until-found"{{/unless}}>
...
<div role="note" class="callout"
    ...
    {{/ifAll}}>
    {{#if title}}
        <h3 class="callout-title"{{#if id}} id="{{id}}"{{/if}}>{{{title}}}</h3>
    {{/if}}
...
`;var A=`{{! valid class values are: up, down, left, right}}
<a class="qld-dir-link {{class}}" 
  ...
  <span class="qld-icon qld-icon-md qld-icon-arrow-{{class}}" aria-hidden="true"></span>
</a>`;var D=`<div class="col-12 col-md-6 col-lg-12">
    ...
    {{#each followLinks }}
      <li>
          <a class="nav-link" href="{{ link }}" {{#if target}}target="{{ target }}"{{/if}} aria-label="{{isdefined accessibleLabel label}}">
              <span class="qld-icon qld-icon-{{icon}}" aria-hidden="true"></span>
    ...
    <div class="row">
      ...
      <div class="col-12 col-lg-3 py-24 py-lg-0 footer-contact {{#if contact.hasBorderColumn}}border-column{{/if}}">
        ...
        <div class="row ">
          ...
          <div class="col-12 col-md-4 col-lg-12 contact-btn order-lg-last d-flex d-lg-block mt-lg-32 ">
            {{!-- Contact Us Button --}}
      ...
      <!-- Optional Column 1 -->
      <nav class="col-12 col-lg-2 optional-column-1 {{#if optionalColumn1.hasBorderColumn}}border-column{{/if}}"
        aria-labelledby="footer-optional-column-1-heading">
        
        <div class="footer-column pe-16">
          {{#if optionalColumn1.title}}
            <h3 class="footer-heading" id="footer-optional-column-1-heading">{{optionalColumn1.title}}</h3>
          {{ else }}
            <h3 class="sr-only" id="footer-optional-column-1-heading">Footer links</h3>
          {{/if}}
      ...
      <!-- WoG links -->
      <nav class="col-12 col-lg-2 {{#if footerLinks.hasBorderColumn}}border-column{{/if}}" aria-labelledby="footer-wog-links-heading">
      ...
      <!-- Optional Column 2 -->
      <nav class="col-12 col-lg-2 optional-column-2 {{#if optionalColumn2.hasBorderColumn}}border-column{{/if}}" aria-labelledby="footer-optional-column-2-heading">

        <div class="footer-column pe-lg-16">
          
          {{#if optionalColumn2.title}}
          <h3 class="footer-heading" id="footer-optional-column-2-heading">{{optionalColumn2.title}}</h3>
          {{else}}
          <h3 class="sr-only" id="footer-optional-column-2-heading">Footer links</h3>
          {{/if}}
...
<!-- VERSION_DETAILS={"project_id":"@qld-gov-au/qgds-bootstrap5","version":"2.4.0","branch":"QGDS-1179-mobile-menu-close-button-aria-label","tag":"","commit":"f0e5ad3ff01acaba379ab9e7b60adb7042440923","majorVersion":"v2"} -->
...
</html>`;var Q=`<svg class="qld-icon-{{size}}"{{#if ariaLabel}} aria-label="{{ariaLabel}}"{{/if}}{{#if ariaHidden}} aria-hidden="{{ariaHidden}}"{{/if}}>
...
{{#if src}}
  ...
  {{/ifAny}}
{{/if}}
`;var j=`<div class="alert {{variantClass}} {{customClass}}" role="alert"
    {{#if ariaLabel}}
...
>
    ...
    {{/if}}
    {{{ content }}}
</div>`;var W=`<!-- QGDS Partial: inpage-nav -->
...
{{/inline}}`;var U=`{{#ifCond logo "===" "coa-landscape"}}
    {{>logoCOALandscape}}
...
{{> footer footer}}`;var e1=`<!-- DCTERMS https://www.dublincore.org/specifications/dublin-core/dcmi-terms/-->
{{#if dcTerms}}
...
{{#if og.type }}<meta property="og:type" content="{{og.type}}" />{{/if}}`;var a1=`<!-- Example button to trigger the modal -->
...
</div>`;var t1=`<!-- QGDS Partial: Primary navigation -->

<nav id="main-nav" class="navbar navbar-expand-lg{{#if metadata.alternativeColor}} dark{{/if}}{{#if metadata.verticalOrientation}} vertical{{/if}}" aria-label="Main" >
    <div id="navbarNav" class="container">
            <div class="nav-header">
                ...
                <button id="burgerCloseBtn" type="button" class="navbar-btn" data-bs-toggle="collapse"
                    ...
                    aria-controls="main-nav">Close</button>
            </div>
        ...
        <div id="overlay"></div>
</nav>`;var C1=`<nav aria-label="{{ arialabel }}">
...
</nav>`;var s1=`<section class="qld-promo-panel {{#ifCond type '==' 'indent-text'}}indent{{/ifCond}} {{#ifCond type '==' 'indent-img'}}indent{{/ifCond}} {{type}}">
...
</section>`;var l1=`<!-- QGDS Component: Loading Quick exit -->
...
`;var r1=`<nav class="qld-side-navigation" aria-label="Side Navigation">
  <button
...
`;var c1=`<nav class="qld-skip-links" aria-label="skip to content links">
  {{#each skipLinks}}
    <a class="qld-skip-links__item" href="#{{targetId}}">{{label}}</a>
  {{/each}}
</nav>`;var f1=`<!-- QGDS Component: Loading Spinner -->
...
`;var g1=`<!-- QGDS Component: Textarea -->
...
<input id={{id}} {{#if value}}value="{{value}}"{{/if}}  class="form-control {{customClass}} {{#if isFilled}}is-filled{{/if}} {{#if isValid}}is-valid{{else}}{{#ifCond isValid "===" false}}is-invalid{{/ifCond}}{{/if}}"
    ...
    isRequired}}required{{/if}} {{#if hint-text}}aria-describedby="{{id}}-hint"{{/if}} />`;var h1="<!-- QGDS Partial: typography -->";var b1=`<!-- QGDS Component: Video -->
...
//# @qld-gov-au/qgds-bootstrap5 - v2.4.0
