"use strict";(()=>{function v2(C,t,i,e){switch(t){case"==":return C==i?e.fn(this):e.inverse(this);case"===":return C===i?e.fn(this):e.inverse(this);case"!=":return C!=i?e.fn(this):e.inverse(this);case"!==":return C!==i?e.fn(this):e.inverse(this);case"<":return C<i?e.fn(this):e.inverse(this);case"<=":return C<=i?e.fn(this):e.inverse(this);case">":return C>i?e.fn(this):e.inverse(this);case">=":return C>=i?e.fn(this):e.inverse(this);case"&&":return C&&i?e.fn(this):e.inverse(this);case"||":return C||i?e.fn(this):e.inverse(this);case"contains":return typeof C=="string"&&typeof i=="string"?C.toLowerCase().indexOf(i.toLowerCase())>=0?e.fn(this):e.inverse(this):e.inverse(this);default:return e.inverse(this)}}function n(C){C.registerHelper("contains",function(t,i,e){return t=C.escapeExpression(t),i=C.escapeExpression(i),i.indexOf(t)>-1?e.fn(this):e.inverse(this)}),C.registerHelper("ifCond",v2),C.registerHelper("isType",function(t,i,e){return t===i?e.fn(this):e.inverse(this)}),C.registerHelper("ifAny",function(...t){let i=t.pop();return t.some(e=>!!e)?i.fn(this):i.inverse(this)}),C.registerHelper("now",function(){return new Date().toISOString()}),C.registerHelper("formatDate",function(t,i,e){let a;if(t&&(a=new Date(t)),isNaN(a)&&i&&(a=new Date(i)),isNaN(a))return"Invalid Date";var l=["January","February","March","April","May","June","July","August","September","October","November","December"],s=a.getDate(),o=l[a.getMonth()],r=a.getFullYear();switch(e){case"YYYY":return`${r}`;case"MMMM YYYY":return`${o} ${r}`;default:return`${s} ${o} ${r}`}}),C.registerHelper("formatDateOrToday",function(t,i){let e=t||new Date().toISOString();return C.helpers.formatDate(t,e,i)}),C.registerHelper("formatDuration",function(t,i){if(!t)return"";if(typeof t=="string"&&i!=="long")return t;let e="",a=[],l,s,o;if(typeof t=="string"){let r=t.split(":");o=r[0],r.length==2?[s="",o=""]=r:r.length==3&&([l="",s="",o=""]=r)}else[l="",s="",o=""]=t;return i==="long"?(l>0&&a.push(`${l} hour${l>1?"s":""}`),s>0&&a.push(`${s} minute${s>1?"s":""}`),o>0&&a.push(`${o} second${o>1?"s":""}`),e=a.join(" ")):(l>0&&a.push(l.toString().padStart(2,0)),a.push(s.toString().padStart(2,0)),a.push(o.toString().padStart(2,0)),e=a.join(":")),e}),C.registerHelper("isdefined",function(t,i){return t!==void 0&&t!==""?t:i}),C.registerHelper("getClassNames",function(t,i){let e=t.split(",").map(l=>l.trim()),a=[];for(let l=0;l<e.length;l++){let s=e[l];if(i){for(let o=0;o<i.length;o++)if(i[o][s]===!0){a.push(s);break}}}return a.length>0?a.join(" "):""}),C.registerHelper("join",function(t,i){return(!i||typeof i!="string")&&(i=" "),Array.isArray(t)?t.join(i):t}),C.registerHelper("toCamelCase",function(t){return typeof t!="string"?t:t.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w|\s+)/g,(i,e)=>e===0?i.toLowerCase():i.toUpperCase()).replace(/\s+/g,"")})}typeof Handlebars!="undefined"&&n(Handlebars);var c=`<!-- QGDS Component: Accordion -->
...
</figure>`;var u=`<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    {{#each breadcrumbs}}
...
<!-- HEADER END -->`;var D=`{{!-- Link icon partial --}}
{{#*inline "linkIcon"~}}
<span class="qld-icon qld-icon-md {{iconClass}} {{iconPosition}}" aria-hidden="true"></span>
{{~/inline}} 
...
{{else}}
    {{~> link~}}
{{/if}}`;var O=`<svg class="qld-icon-{{size}}"{{#if ariaLabel}} aria-label="{{ariaLabel}}"{{/if}}{{#if ariaHidden}} aria-hidden="{{ariaHidden}}"{{/if}}>
  <use href="./assets/img/icons-sprite.svg#qgds-icon-{{iconId}}"></use>
</svg>`;var R=`{{! Image template }}
{{#*inline "imageTag"}}
...
{{/inline}}`;var N=`{{#ifCond logo "===" "coa-landscape"}}
    {{>logoCOALandscape}}
...
</div>`;var Y=`<!-- QGDS Partial: Primary navigation -->
...
<!-- MAIN NAVIGATION END -->`;var U=`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</nav>`;var C2=`<section class="qld-promo-panel {{#ifCond type '==' 'indent-text'}}indent{{/ifCond}} {{#ifCond type '==' 'indent-img'}}indent{{/ifCond}} {{type}}">
...
</section>`;var e2=`<!-- QGDS Component: Loading Quick exit -->
...
`;var a2=`<div class="qld-side-navigation">
...
`;var o2=`<nav class="qld-skip-links" aria-label="skip to content links">
  {{#each skipLinks}}
    <a class="qld-skip-links__item" href="#{{targetId}}">{{label}}</a>
  {{/each}}
</nav>`;var s2=`<!-- QGDS Component: Loading Spinner -->
...
</section>`;var d2=`<!-- QGDS Component: Tag -->
...
`;var c2=`<!-- QGDS Component: Textarea -->
...
<textarea id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    aria-required="true" {{/if}}></textarea>`;var p2=`<!-- QGDS Component: Textbox -->
...
<input id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    isRequired}}required aria-required="true" {{/if}} />`;var f2="<!-- QGDS Partial: typography -->";var h2=`<!-- QGDS Component: Video -->
...
`;function d(C){C.registerPartial("accordion",c),C.registerPartial("backToTop",p),C.registerPartial("banner",f),C.registerPartial("blockquote",h),C.registerPartial("breadcrumbs",u),C.registerPartial("breadcrumbsWrapper",L),C.registerPartial("button",v),C.registerPartial("callToAction",m),C.registerPartial("callout",g),C.registerPartial("card",V),C.registerPartial("containerLayout",H),C.registerPartial("contentFooter",M),C.registerPartial("contentFooterWrapper",b),C.registerPartial("contentWrapper",Z),C.registerPartial("correctincorrect",_),C.registerPartial("dateinput",w),C.registerPartial("directionLinks",y),C.registerPartial("footer",k),C.registerPartial("footerForgov",q),C.registerPartial("formcheck",x),C.registerPartial("fullPage",T),C.registerPartial("globalAlert",S),C.registerPartial("head",P),C.registerPartial("header",A),C.registerPartial("iconLink",D),C.registerPartial("icon",O),C.registerPartial("image",R),C.registerPartial("inpageAlert",I),C.registerPartial("inpagenav",E),C.registerPartial("linkColumns",F),C.registerPartial("logo",N),C.registerPartial("logoCOALandscape",G),C.registerPartial("logoCOALandscape2Lines",Q),C.registerPartial("mainContainerWrapper",B),C.registerPartial("metaDcTerms",W),C.registerPartial("metaOpenGraph",j),C.registerPartial("modal",J),C.registerPartial("navbar",Y),C.registerPartial("pageLayout",U),C.registerPartial("contentPageWithForm",$),C.registerPartial("contentPageWithSideNavigation",z),C.registerPartial("homePage",X),C.registerPartial("pagination",K),C.registerPartial("promotionalPanel",C2),C.registerPartial("quickexit",e2),C.registerPartial("searchInput",i2),C.registerPartial("select",t2),C.registerPartial("sidenav",a2),C.registerPartial("sidenavWrapper",l2),C.registerPartial("skipLinks",o2),C.registerPartial("spinner",s2),C.registerPartial("table",r2),C.registerPartial("tabs",n2),C.registerPartial("tag",d2),C.registerPartial("textarea",c2),C.registerPartial("textbox",p2),C.registerPartial("typography",f2),C.registerPartial("video",h2)}typeof Handlebars!="undefined"&&d(Handlebars);var u2=!1;function L2(C){typeof C=="undefined"&&console.error("Handlebars.init requires HandleBars"),typeof C!="undefined"?(u2?console.log("HandleBars Helpers And Partials already loaded, loading again"):u2=!0,n(C),d(C)):console.log("Handlebars not found, init failed")}typeof Handlebars!="undefined"&&L2(Handlebars);})();
