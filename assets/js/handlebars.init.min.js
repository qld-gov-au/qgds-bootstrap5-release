"use strict";(()=>{function p2(C,a,t,e){switch(a){case"==":return C==t?e.fn(this):e.inverse(this);case"===":return C===t?e.fn(this):e.inverse(this);case"!=":return C!=t?e.fn(this):e.inverse(this);case"!==":return C!==t?e.fn(this):e.inverse(this);case"<":return C<t?e.fn(this):e.inverse(this);case"<=":return C<=t?e.fn(this):e.inverse(this);case">":return C>t?e.fn(this):e.inverse(this);case">=":return C>=t?e.fn(this):e.inverse(this);case"&&":return C&&t?e.fn(this):e.inverse(this);case"||":return C||t?e.fn(this):e.inverse(this);case"contains":return typeof C=="string"&&typeof t=="string"?C.toLowerCase().indexOf(t.toLowerCase())>=0?e.fn(this):e.inverse(this):e.inverse(this);default:return e.inverse(this)}}function n(C){C.registerHelper("contains",function(a,t,e){return a=C.escapeExpression(a),t=C.escapeExpression(t),t.indexOf(a)>-1?e.fn(this):e.inverse(this)}),C.registerHelper("ifCond",p2),C.registerHelper("isType",function(a,t,e){return a===t?e.fn(this):e.inverse(this)}),C.registerHelper("ifAny",function(...a){let t=a.pop();return a.some(e=>!!e)?t.fn(this):t.inverse(this)}),C.registerHelper("now",function(){return new Date().toISOString()}),C.registerHelper("formatDate",function(a,t,e){let i;if(a&&(i=new Date(a)),isNaN(i)&&t&&(i=new Date(t)),isNaN(i))return"Invalid Date";var l=["January","February","March","April","May","June","July","August","September","October","November","December"],r=i.getDate(),o=l[i.getMonth()],s=i.getFullYear();switch(e){case"YYYY":return`${s}`;case"MMMM YYYY":return`${o} ${s}`;default:return`${r} ${o} ${s}`}}),C.registerHelper("formatDateOrToday",function(a,t){let e=a||new Date().toISOString();return C.helpers.formatDate(a,e,t)}),C.registerHelper("formatDuration",function(a,t){if(!a)return"";if(typeof a=="string"&&t!=="long")return a;let e="",i=[],l,r,o;if(typeof a=="string"){let s=a.split(":");o=s[0],s.length==2?[r="",o=""]=s:s.length==3&&([l="",r="",o=""]=s)}else[l="",r="",o=""]=a;return t==="long"?(l>0&&i.push(`${l} hour${l>1?"s":""}`),r>0&&i.push(`${r} minute${r>1?"s":""}`),o>0&&i.push(`${o} second${o>1?"s":""}`),e=i.join(" ")):(l>0&&i.push(l.toString().padStart(2,0)),i.push(r.toString().padStart(2,0)),i.push(o.toString().padStart(2,0)),e=i.join(":")),e}),C.registerHelper("isdefined",function(a,t){return a!==void 0&&a!==""?a:t}),C.registerHelper("getClassNames",function(a,t){let e=a.split(",").map(l=>l.trim()),i=[];for(let l=0;l<e.length;l++){let r=e[l];if(t){for(let o=0;o<t.length;o++)if(t[o][r]===!0){i.push(r);break}}}return i.length>0?i.join(" "):""}),C.registerHelper("join",function(a,t){return(!t||typeof t!="string")&&(t=" "),Array.isArray(a)?a.join(t):a}),C.registerHelper("toCamelCase",function(a){return typeof a!="string"?a:a.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w|\s+)/g,(t,e)=>e===0?t.toLowerCase():t.toUpperCase()).replace(/\s+/g,"")})}typeof Handlebars!="undefined"&&n(Handlebars);var c=`<!-- QGDS Component: Accordion -->
...
<!-- HEADER END -->`;var R=`<svg class="qld-icon-{{size}}"{{#if ariaLabel}} aria-label="{{ariaLabel}}"{{/if}}{{#if ariaHidden}} aria-hidden="{{ariaHidden}}"{{/if}}>
...
{{/if}}`;var O=`<div class="alert {{variantClass}} {{customClass}}" role="alert" 
    ...
    {{/if}}
    {{{ content }}}
</div>`;var P=`<!-- QGDS Partial: inpage-nav -->
...
</div>`;var W=`<!-- QGDS Partial: Primary navigation -->
...
</nav>`;var U=`<section class="qld-promo-panel {{#ifCond type '==' 'indent-text'}}indent{{/ifCond}} {{#ifCond type '==' 'indent-img'}}indent{{/ifCond}} {{type}}">
...
</section>`;var $=`<!-- QGDS Component: Loading Quick exit -->
...
`;var K=`<div class="qld-side-navigation">
...
`;var e2=`<nav class="qld-skip-links" aria-label="skip to content links">
  {{#each skipLinks}}
    <a class="qld-skip-links__item" href="#{{targetId}}">{{label}}</a>
  {{/each}}
</nav>`;var t2=`<!-- QGDS Component: Loading Spinner -->
...
</section>`;var l2=`<!-- QGDS Component: Tag -->
...
`;var o2=`<!-- QGDS Component: Textarea -->
...
<textarea id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    aria-required="true" {{/if}}></textarea>`;var r2=`<!-- QGDS Component: Textbox -->
...
<input id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    isRequired}}required aria-required="true" {{/if}} />`;var s2="<!-- QGDS Partial: typography -->";var n2=`<!-- QGDS Component: Video -->
...
`;function d(C){C.registerPartial("accordion",c),C.registerPartial("backToTop",p),C.registerPartial("banner",f),C.registerPartial("blockquote",L),C.registerPartial("breadcrumbs",h),C.registerPartial("breadcrumbsWrapper",u),C.registerPartial("button",v),C.registerPartial("callToAction",g),C.registerPartial("callout",m),C.registerPartial("card",V),C.registerPartial("contentFooter",H),C.registerPartial("contentFooterWrapper",M),C.registerPartial("contentWrapper",Z),C.registerPartial("correctincorrect",b),C.registerPartial("dateinput",_),C.registerPartial("directionLinks",w),C.registerPartial("footer",y),C.registerPartial("footerForgov",k),C.registerPartial("formcheck",q),C.registerPartial("fullPage",x),C.registerPartial("globalAlert",T),C.registerPartial("head",S),C.registerPartial("header",A),C.registerPartial("icon",R),C.registerPartial("image",D),C.registerPartial("inpageAlert",O),C.registerPartial("inpagenav",P),C.registerPartial("linkColumns",E),C.registerPartial("logo",I),C.registerPartial("logoCOALandscape",F),C.registerPartial("logoCOALandscape2Lines",N),C.registerPartial("mainContainerWrapper",G),C.registerPartial("metaDcTerms",Q),C.registerPartial("metaOpenGraph",B),C.registerPartial("modal",J),C.registerPartial("navbar",W),C.registerPartial("pageLayout",Y),C.registerPartial("pagination",j),C.registerPartial("promotionalPanel",U),C.registerPartial("quickexit",$),C.registerPartial("searchInput",z),C.registerPartial("select",X),C.registerPartial("sidenav",K),C.registerPartial("sidenavWrapper",C2),C.registerPartial("skipLinks",e2),C.registerPartial("spinner",t2),C.registerPartial("table",a2),C.registerPartial("tabs",i2),C.registerPartial("tag",l2),C.registerPartial("textarea",o2),C.registerPartial("textbox",r2),C.registerPartial("typography",s2),C.registerPartial("video",n2)}typeof Handlebars!="undefined"&&d(Handlebars);var d2=!1;function c2(C){typeof C=="undefined"&&console.error("Handlebars.init requires HandleBars"),typeof C!="undefined"?(d2?console.log("HandleBars Helpers And Partials already loaded, loading again"):d2=!0,n(C),d(C)):console.log("Handlebars not found, init failed")}typeof Handlebars!="undefined"&&c2(Handlebars);})();
