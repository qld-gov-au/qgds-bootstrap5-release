"use strict";(()=>{function h2(C,i,t,e){switch(i){case"==":return C==t?e.fn(this):e.inverse(this);case"===":return C===t?e.fn(this):e.inverse(this);case"!=":return C!=t?e.fn(this):e.inverse(this);case"!==":return C!==t?e.fn(this):e.inverse(this);case"<":return C<t?e.fn(this):e.inverse(this);case"<=":return C<=t?e.fn(this):e.inverse(this);case">":return C>t?e.fn(this):e.inverse(this);case">=":return C>=t?e.fn(this):e.inverse(this);case"&&":return C&&t?e.fn(this):e.inverse(this);case"||":return C||t?e.fn(this):e.inverse(this);case"contains":return typeof C=="string"&&typeof t=="string"?C.toLowerCase().indexOf(t.toLowerCase())>=0?e.fn(this):e.inverse(this):e.inverse(this);default:return e.inverse(this)}}function n(C){C.registerHelper("contains",function(i,t,e){return i=C.escapeExpression(i),t=C.escapeExpression(t),t.indexOf(i)>-1?e.fn(this):e.inverse(this)}),C.registerHelper("ifCond",h2),C.registerHelper("isType",function(i,t,e){return i===t?e.fn(this):e.inverse(this)}),C.registerHelper("ifAny",function(...i){let t=i.pop();return i.some(e=>!!e)?t.fn(this):t.inverse(this)}),C.registerHelper("now",function(){return new Date().toISOString()}),C.registerHelper("formatDate",function(i,t,e){let a;if(i&&(a=new Date(i)),isNaN(a)&&t&&(a=new Date(t)),isNaN(a))return"Invalid Date";var l=["January","February","March","April","May","June","July","August","September","October","November","December"],r=a.getDate(),o=l[a.getMonth()],s=a.getFullYear();switch(e){case"YYYY":return`${s}`;case"MMMM YYYY":return`${o} ${s}`;default:return`${r} ${o} ${s}`}}),C.registerHelper("formatDateOrToday",function(i,t){let e=i||new Date().toISOString();return C.helpers.formatDate(i,e,t)}),C.registerHelper("formatDuration",function(i,t){if(!i)return"";if(typeof i=="string"&&t!=="long")return i;let e="",a=[],l,r,o;if(typeof i=="string"){let s=i.split(":");o=s[0],s.length==2?[r="",o=""]=s:s.length==3&&([l="",r="",o=""]=s)}else[l="",r="",o=""]=i;return t==="long"?(l>0&&a.push(`${l} hour${l>1?"s":""}`),r>0&&a.push(`${r} minute${r>1?"s":""}`),o>0&&a.push(`${o} second${o>1?"s":""}`),e=a.join(" ")):(l>0&&a.push(l.toString().padStart(2,0)),a.push(r.toString().padStart(2,0)),a.push(o.toString().padStart(2,0)),e=a.join(":")),e}),C.registerHelper("isdefined",function(i,t){return i!==void 0&&i!==""?i:t}),C.registerHelper("getClassNames",function(i,t){let e=i.split(",").map(l=>l.trim()),a=[];for(let l=0;l<e.length;l++){let r=e[l];if(t){for(let o=0;o<t.length;o++)if(t[o][r]===!0){a.push(r);break}}}return a.length>0?a.join(" "):""}),C.registerHelper("join",function(i,t){return(!t||typeof t!="string")&&(t=" "),Array.isArray(i)?i.join(t):i}),C.registerHelper("toCamelCase",function(i){return typeof i!="string"?i:i.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w|\s+)/g,(t,e)=>e===0?t.toLowerCase():t.toUpperCase()).replace(/\s+/g,"")})}typeof Handlebars!="undefined"&&n(Handlebars);var c=`<!-- QGDS Component: Accordion -->
...
{{/unless}}`;var f=`<!-- QGDS Banner Component -->
  ...
  </div>`;var L=`<!-- QGDS Component: Blockquote -->
...
{{/unless}}`;var m=`<a class="qld-cta-link {{getClassNames "small, view-all" class}}"
    {{#if id}}id="{{id}}"{{/if}}
    href="{{href}}" 
    target="{{target}}" 
...
</a>`;var g=`<!-- QGDS Component: Callout -->
...
{{/if}}`;var x=`<footer class="qld-footer {{ variantClass }}" role="contentinfo">
...
</footer>`;var T=`<footer class="qld-footer {{ variantClass }}" role="contentinfo">
  <!-- Footer content container -->
    <div class="container">
        <div class="row">
            <div class="col title">
...
</html>`;var P=`<!-- QGDS Component: Global Alert -->
...
`;var O=`<header class="qld__header" role="banner">
...
{{#if src}}
  ...
  {{else}}
    {{> imageTag}}
  {{/if}}
{{/if}}`;var I=`<div class="alert {{variantClass}} {{customClass}}" role="alert" 
    ...
    {{/if}}
    {{{ content }}}
</div>`;var F=`<!-- QGDS Partial: inpage-nav -->
...
{{/inline}}`;var G=`{{#ifCond logo "===" "coa-landscape"}}
    {{>logoCOALandscape}}
...
{{> footer footer}}`;var W=`<!-- DCTERMS https://www.dublincore.org/specifications/dublin-core/dcmi-terms/-->
{{#if dcTerms}}
...
{{#if og.type }}<meta property="og:type" content="{{og.type}}" />{{/if}}`;var j=`<!-- Example button to trigger the modal -->
...
</div>`;var U=`<!-- QGDS Partial: Primary navigation -->
...
<!-- MAIN NAVIGATION END -->`;var $=`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</nav>`;var X=`<section class="qld-promo-panel {{#ifCond type '==' 'indent-text'}}indent{{/ifCond}} {{#ifCond type '==' 'indent-img'}}indent{{/ifCond}} {{type}}">
...
</section>`;var K=`<!-- QGDS Component: Loading Quick exit -->
...
`;var t2=`<div class="qld-side-navigation">
...
`;var a2=`<nav class="qld-skip-links" aria-label="skip to content links">
  {{#each skipLinks}}
    <a class="qld-skip-links__item" href="#{{targetId}}">{{label}}</a>
  {{/each}}
</nav>`;var l2=`<!-- QGDS Component: Loading Spinner -->
...
</section>`;var s2=`<!-- QGDS Component: Tag -->
...
`;var n2=`<!-- QGDS Component: Textarea -->
...
<textarea id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    aria-required="true" {{/if}}></textarea>`;var d2=`<!-- QGDS Component: Textbox -->
...
<input id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    isRequired}}required aria-required="true" {{/if}} />`;var c2="<!-- QGDS Partial: typography -->";var p2=`<!-- QGDS Component: Video -->
...
`;function d(C){C.registerPartial("accordion",c),C.registerPartial("backToTop",p),C.registerPartial("banner",f),C.registerPartial("blockquote",L),C.registerPartial("breadcrumbs",h),C.registerPartial("breadcrumbsWrapper",u),C.registerPartial("button",v),C.registerPartial("callToAction",m),C.registerPartial("callout",g),C.registerPartial("card",V),C.registerPartial("contentFooter",H),C.registerPartial("contentFooterWrapper",M),C.registerPartial("contentWrapper",Z),C.registerPartial("correctincorrect",b),C.registerPartial("dateinput",_),C.registerPartial("directionLinks",w),C.registerPartial("customLinks",k),C.registerPartial("feedbackForm",y),C.registerPartial("followLinks",q),C.registerPartial("footer",x),C.registerPartial("footerForgov",T),C.registerPartial("formcheck",S),C.registerPartial("fullPage",A),C.registerPartial("globalAlert",P),C.registerPartial("head",R),C.registerPartial("header",O),C.registerPartial("icon",D),C.registerPartial("image",E),C.registerPartial("inpageAlert",I),C.registerPartial("inpagenav",F),C.registerPartial("linkColumns",N),C.registerPartial("logo",G),C.registerPartial("logoCOALandscape",Q),C.registerPartial("logoCOALandscape2Lines",B),C.registerPartial("mainContainerWrapper",J),C.registerPartial("metaDcTerms",W),C.registerPartial("metaOpenGraph",Y),C.registerPartial("modal",j),C.registerPartial("navbar",U),C.registerPartial("pageLayout",$),C.registerPartial("pagination",z),C.registerPartial("promotionalPanel",X),C.registerPartial("quickexit",K),C.registerPartial("searchInput",C2),C.registerPartial("select",e2),C.registerPartial("sidenav",t2),C.registerPartial("sidenavWrapper",i2),C.registerPartial("skipLinks",a2),C.registerPartial("spinner",l2),C.registerPartial("table",o2),C.registerPartial("tabs",r2),C.registerPartial("tag",s2),C.registerPartial("textarea",n2),C.registerPartial("textbox",d2),C.registerPartial("typography",c2),C.registerPartial("video",p2)}typeof Handlebars!="undefined"&&d(Handlebars);var f2=!1;function L2(C){typeof C=="undefined"&&console.error("Handlebars.init requires HandleBars"),typeof C!="undefined"?(f2?console.log("HandleBars Helpers And Partials already loaded, loading again"):f2=!0,n(C),d(C)):console.log("Handlebars not found, init failed")}typeof Handlebars!="undefined"&&L2(Handlebars);})();
