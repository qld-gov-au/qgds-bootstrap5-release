"use strict";(()=>{function g2(C,a,i,e){switch(a){case"==":return C==i?e.fn(this):e.inverse(this);case"===":return C===i?e.fn(this):e.inverse(this);case"!=":return C!=i?e.fn(this):e.inverse(this);case"!==":return C!==i?e.fn(this):e.inverse(this);case"<":return C<i?e.fn(this):e.inverse(this);case"<=":return C<=i?e.fn(this):e.inverse(this);case">":return C>i?e.fn(this):e.inverse(this);case">=":return C>=i?e.fn(this):e.inverse(this);case"&&":return C&&i?e.fn(this):e.inverse(this);case"||":return C||i?e.fn(this):e.inverse(this);case"contains":return typeof C=="string"&&typeof i=="string"?C.toLowerCase().indexOf(i.toLowerCase())>=0?e.fn(this):e.inverse(this):e.inverse(this);default:return e.inverse(this)}}function r(C){C.registerHelper("contains",function(a,i,e){return a=C.escapeExpression(a),i=C.escapeExpression(i),i.indexOf(a)>-1?e.fn(this):e.inverse(this)}),C.registerHelper("ifCond",g2),C.registerHelper("isType",function(a,i,e){return a===i?e.fn(this):e.inverse(this)}),C.registerHelper("ifAny",function(...a){let i=a.pop();return a.some(e=>!!e)?i.fn(this):i.inverse(this)}),C.registerHelper("now",function(){return new Date().toISOString()}),C.registerHelper("formatDate",function(a,i,e){let t;if(a&&(t=new Date(a)),isNaN(t)&&i&&(t=new Date(i)),isNaN(t))return"Invalid Date";var s=["January","February","March","April","May","June","July","August","September","October","November","December"],n=t.getDate(),l=s[t.getMonth()],o=t.getFullYear();switch(e){case"YYYY":return`${o}`;case"MMMM YYYY":return`${l} ${o}`;default:return`${n} ${l} ${o}`}}),C.registerHelper("formatDateOrToday",function(a,i){let e=a||new Date().toISOString();return C.helpers.formatDate(a,e,i)}),C.registerHelper("formatDuration",function(a,i){if(!a)return"";if(typeof a=="string"&&i!=="long")return a;let e="",t=[],s,n,l;if(typeof a=="string"){let o=a.split(":");l=o[0],o.length==2?[n="",l=""]=o:o.length==3&&([s="",n="",l=""]=o)}else[s="",n="",l=""]=a;return i==="long"?(s>0&&t.push(`${s} hour${s>1?"s":""}`),n>0&&t.push(`${n} minute${n>1?"s":""}`),l>0&&t.push(`${l} second${l>1?"s":""}`),e=t.join(" ")):(s>0&&t.push(s.toString().padStart(2,0)),t.push(n.toString().padStart(2,0)),t.push(l.toString().padStart(2,0)),e=t.join(":")),e}),C.registerHelper("isdefined",function(a,i){return a!==void 0&&a!==""?a:i}),C.registerHelper("getClassNames",function(a,i){let e=a.split(",").map(s=>s.trim()),t=[];for(let s=0;s<e.length;s++){let n=e[s];if(i){for(let l=0;l<i.length;l++)if(i[l][n]===!0){t.push(n);break}}}return t.length>0?t.join(" "):""}),C.registerHelper("join",function(a,i){return(!i||typeof i!="string")&&(i=" "),Array.isArray(a)?a.join(i):a}),C.registerHelper("toCamelCase",function(a){return typeof a!="string"?a:a.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w|\s+)/g,(i,e)=>e===0?i.toLowerCase():i.toUpperCase()).replace(/\s+/g,"")})}typeof Handlebars!="undefined"&&r(Handlebars);var c=`<!-- QGDS Component: Accordion -->
...
`;var Z="";var k=`<!-- QGDS Component: Date input -->
...
`;var x=`{{! valid class values are: up, down, left, right}}
...
`;var O=`<header class="qld-header" role="banner">
...
</header>`;var A=`<div class="qld-header-brand">
    <a href="{{url}}">
...
{{else}}
    {{~> link~}}
{{/if}}`;var R=`<svg class="qld-icon-{{size}}"{{#if ariaLabel}} aria-label="{{ariaLabel}}"{{/if}}{{#if ariaHidden}} aria-hidden="{{ariaHidden}}"{{/if}}>
  <use href="./assets/img/icons-sprite.svg#qgds-icon-{{iconId}}"></use>
</svg>`;var I=`{{! Image template }}
{{#*inline "imageTag"}}
...
{{> footer footer}}`;var j=`<!-- DCTERMS https://www.dublincore.org/specifications/dublin-core/dcmi-terms/-->
{{#if dcTerms}}
...
</nav>`;var e2=`<section class="qld-promo-panel {{#ifCond type '==' 'indent-text'}}indent{{/ifCond}} {{#ifCond type '==' 'indent-img'}}indent{{/ifCond}} {{type}}">
...
</section>`;var i2=`<!-- QGDS Component: Loading Quick exit -->
...
`;var s2=`<div class="qld-side-navigation">
...
`;var n2=`<nav class="qld-skip-links" aria-label="skip to content links">
  {{#each skipLinks}}
...
`;var f2=`<!-- QGDS Component: Textarea -->
...
<input id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    isRequired}}required aria-required="true" {{/if}} />`;var h2="<!-- QGDS Partial: typography -->";var m2=`<!-- QGDS Component: Video -->
...
`;function d(C){C.registerPartial("accordion",c),C.registerPartial("backToTop",f),C.registerPartial("banner",p),C.registerPartial("blockquote",h),C.registerPartial("breadcrumbs",m),C.registerPartial("breadcrumbsWrapper",u),C.registerPartial("button",v),C.registerPartial("callToAction",g),C.registerPartial("callout",b),C.registerPartial("card",L),C.registerPartial("containerLayout",V),C.registerPartial("contentFooter",M),C.registerPartial("contentFooterWrapper",H),C.registerPartial("contentWrapper",w),C.registerPartial("correctincorrect",Z),C.registerPartial("dateinput",k),C.registerPartial("directionLinks",x),C.registerPartial("footer",y),C.registerPartial("footerForgov",q),C.registerPartial("formcheck",T),C.registerPartial("fullPage",S),C.registerPartial("globalAlert",_),C.registerPartial("head",P),C.registerPartial("header",O),C.registerPartial("headerBrand",A),C.registerPartial("iconLink",D),C.registerPartial("icon",R),C.registerPartial("image",I),C.registerPartial("inpageAlert",E),C.registerPartial("inpagenav",F),C.registerPartial("linkColumns",N),C.registerPartial("logo",G),C.registerPartial("logoCOALandscape",B),C.registerPartial("logoCOALandscape2Lines",W),C.registerPartial("mainContainerWrapper",Q),C.registerPartial("metaDcTerms",j),C.registerPartial("metaOpenGraph",J),C.registerPartial("modal",Y),C.registerPartial("navbar",U),C.registerPartial("pageLayout",$),C.registerPartial("contentPageWithForm",z),C.registerPartial("contentPageWithSideNavigation",X),C.registerPartial("fullWidthLandingPage",K),C.registerPartial("pagination",C2),C.registerPartial("promotionalPanel",e2),C.registerPartial("quickexit",i2),C.registerPartial("searchInput",a2),C.registerPartial("select",t2),C.registerPartial("sidenav",s2),C.registerPartial("sidenavWrapper",l2),C.registerPartial("skipLinks",n2),C.registerPartial("spinner",o2),C.registerPartial("table",r2),C.registerPartial("tabs",d2),C.registerPartial("tag",c2),C.registerPartial("textarea",f2),C.registerPartial("textbox",p2),C.registerPartial("typography",h2),C.registerPartial("video",m2)}typeof Handlebars!="undefined"&&d(Handlebars);var u2=!1;function v2(C){typeof C=="undefined"&&console.error("Handlebars.init requires HandleBars"),typeof C!="undefined"?(u2?console.log("HandleBars Helpers And Partials already loaded, loading again"):u2=!0,r(C),d(C)):console.log("Handlebars not found, init failed")}typeof Handlebars!="undefined"&&v2(Handlebars);})();
