"use strict";(()=>{function u2(C,i,t,e){switch(i){case"==":return C==t?e.fn(this):e.inverse(this);case"===":return C===t?e.fn(this):e.inverse(this);case"!=":return C!=t?e.fn(this):e.inverse(this);case"!==":return C!==t?e.fn(this):e.inverse(this);case"<":return C<t?e.fn(this):e.inverse(this);case"<=":return C<=t?e.fn(this):e.inverse(this);case">":return C>t?e.fn(this):e.inverse(this);case">=":return C>=t?e.fn(this):e.inverse(this);case"&&":return C&&t?e.fn(this):e.inverse(this);case"||":return C||t?e.fn(this):e.inverse(this);case"contains":return typeof C=="string"&&typeof t=="string"?C.toLowerCase().indexOf(t.toLowerCase())>=0?e.fn(this):e.inverse(this):e.inverse(this);default:return e.inverse(this)}}function n(C){C.registerHelper("contains",function(i,t,e){return i=C.escapeExpression(i),t=C.escapeExpression(t),t.indexOf(i)>-1?e.fn(this):e.inverse(this)}),C.registerHelper("ifCond",u2),C.registerHelper("isType",function(i,t,e){return i===t?e.fn(this):e.inverse(this)}),C.registerHelper("ifAny",function(...i){let t=i.pop();return i.some(e=>!!e)?t.fn(this):t.inverse(this)}),C.registerHelper("now",function(){return new Date().toISOString()}),C.registerHelper("formatDate",function(i,t,e){let a;if(i&&(a=new Date(i)),isNaN(a)&&t&&(a=new Date(t)),isNaN(a))return"Invalid Date";var l=["January","February","March","April","May","June","July","August","September","October","November","December"],s=a.getDate(),r=l[a.getMonth()],o=a.getFullYear();switch(e){case"YYYY":return`${o}`;case"MMMM YYYY":return`${r} ${o}`;default:return`${s} ${r} ${o}`}}),C.registerHelper("formatDateOrToday",function(i,t){let e=i||new Date().toISOString();return C.helpers.formatDate(i,e,t)}),C.registerHelper("formatDuration",function(i,t){if(!i)return"";if(typeof i=="string"&&t!=="long")return i;let e="",a=[],l,s,r;if(typeof i=="string"){let o=i.split(":");r=o[0],o.length==2?[s="",r=""]=o:o.length==3&&([l="",s="",r=""]=o)}else[l="",s="",r=""]=i;return t==="long"?(l>0&&a.push(`${l} hour${l>1?"s":""}`),s>0&&a.push(`${s} minute${s>1?"s":""}`),r>0&&a.push(`${r} second${r>1?"s":""}`),e=a.join(" ")):(l>0&&a.push(l.toString().padStart(2,0)),a.push(s.toString().padStart(2,0)),a.push(r.toString().padStart(2,0)),e=a.join(":")),e}),C.registerHelper("isdefined",function(i,t){return i!==void 0&&i!==""?i:t}),C.registerHelper("getClassNames",function(i,t){let e=i.split(",").map(l=>l.trim()),a=[];for(let l=0;l<e.length;l++){let s=e[l];if(t){for(let r=0;r<t.length;r++)if(t[r][s]===!0){a.push(s);break}}}return a.length>0?a.join(" "):""}),C.registerHelper("join",function(i,t){return(!t||typeof t!="string")&&(t=" "),Array.isArray(i)?i.join(t):i}),C.registerHelper("toCamelCase",function(i){return typeof i!="string"?i:i.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w|\s+)/g,(t,e)=>e===0?t.toLowerCase():t.toUpperCase()).replace(/\s+/g,"")})}typeof Handlebars!="undefined"&&n(Handlebars);var c=`<!-- QGDS Component: Accordion -->
...
`;var b=`<div class="container qld-content-footer">
  <div class="row">
    {{> @partial-block }}
  </div>
...
`;var w="";var _=`<!-- QGDS Component: Date input -->
...
`;var y=`{{! valid class values are: up, down, left, right}}
...
</html>`;var S=`<!-- QGDS Component: Global Alert -->
...
<!-- HEADER END -->`;var A=`<svg class="qld-icon-{{size}}"{{#if ariaLabel}} aria-label="{{ariaLabel}}"{{/if}}{{#if ariaHidden}} aria-hidden="{{ariaHidden}}"{{/if}}>
  <use href="./assets/img/icons-sprite.svg#qgds-icon-{{iconId}}"></use>
</svg>`;var D=`{{! Image template }}
{{#*inline "imageTag"}}
...
{{/if}}`;var R=`<div class="alert {{variantClass}} {{customClass}}" role="alert" 
    ...
    {{/if}}
    {{{ content }}}
</div>`;var E=`<!-- QGDS Partial: inpage-nav -->
...
{{/inline}}`;var F=`{{#ifCond logo "===" "coa-landscape"}}
    {{>logoCOALandscape}}
...
{{> footer footer}}`;var Q=`<!-- DCTERMS https://www.dublincore.org/specifications/dublin-core/dcmi-terms/-->
{{#if dcTerms}}
...
</nav>`;var K=`<section class="qld-promo-panel {{#ifCond type '==' 'indent-text'}}indent{{/ifCond}} {{#ifCond type '==' 'indent-img'}}indent{{/ifCond}} {{type}}">
...
</section>`;var C2=`<!-- QGDS Component: Loading Quick exit -->
...
`;var i2=`<div class="qld-side-navigation">
...
`;var l2=`<nav class="qld-skip-links" aria-label="skip to content links">
  {{#each skipLinks}}
    <a class="qld-skip-links__item" href="#{{targetId}}">{{label}}</a>
  {{/each}}
</nav>`;var r2=`<!-- QGDS Component: Loading Spinner -->
...
</section>`;var n2=`<!-- QGDS Component: Tag -->
...
`;var d2=`<!-- QGDS Component: Textarea -->
...
<textarea id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    aria-required="true" {{/if}}></textarea>`;var c2=`<!-- QGDS Component: Textbox -->
...
<input id="example-1" class="qld-text-input form-control {{customClass}} {{#if isFilled}}form-style-filled{{/if}}"
    ...
    isRequired}}required aria-required="true" {{/if}} />`;var f2="<!-- QGDS Partial: typography -->";var p2=`<!-- QGDS Component: Video -->
...
`;function d(C){C.registerPartial("accordion",c),C.registerPartial("backToTop",f),C.registerPartial("banner",p),C.registerPartial("blockquote",L),C.registerPartial("breadcrumbs",h),C.registerPartial("breadcrumbsWrapper",u),C.registerPartial("button",v),C.registerPartial("callToAction",m),C.registerPartial("callout",g),C.registerPartial("card",V),C.registerPartial("containerLayout",H),C.registerPartial("contentFooter",M),C.registerPartial("contentFooterWrapper",b),C.registerPartial("contentWrapper",Z),C.registerPartial("correctincorrect",w),C.registerPartial("dateinput",_),C.registerPartial("directionLinks",y),C.registerPartial("footer",k),C.registerPartial("footerForgov",q),C.registerPartial("formcheck",x),C.registerPartial("fullPage",T),C.registerPartial("globalAlert",S),C.registerPartial("head",P),C.registerPartial("header",O),C.registerPartial("icon",A),C.registerPartial("image",D),C.registerPartial("inpageAlert",R),C.registerPartial("inpagenav",E),C.registerPartial("linkColumns",I),C.registerPartial("logo",F),C.registerPartial("logoCOALandscape",N),C.registerPartial("logoCOALandscape2Lines",G),C.registerPartial("mainContainerWrapper",B),C.registerPartial("metaDcTerms",Q),C.registerPartial("metaOpenGraph",W),C.registerPartial("modal",j),C.registerPartial("navbar",J),C.registerPartial("pageLayout",Y),C.registerPartial("contentPageWithForm",U),C.registerPartial("contentPageWithSideNavigation",$),C.registerPartial("fullWidthLandingPage",z),C.registerPartial("pagination",X),C.registerPartial("promotionalPanel",K),C.registerPartial("quickexit",C2),C.registerPartial("searchInput",e2),C.registerPartial("select",t2),C.registerPartial("sidenav",i2),C.registerPartial("sidenavWrapper",a2),C.registerPartial("skipLinks",l2),C.registerPartial("spinner",r2),C.registerPartial("table",s2),C.registerPartial("tabs",o2),C.registerPartial("tag",n2),C.registerPartial("textarea",d2),C.registerPartial("textbox",c2),C.registerPartial("typography",f2),C.registerPartial("video",p2)}typeof Handlebars!="undefined"&&d(Handlebars);var L2=!1;function h2(C){typeof C=="undefined"&&console.error("Handlebars.init requires HandleBars"),typeof C!="undefined"?(L2?console.log("HandleBars Helpers And Partials already loaded, loading again"):L2=!0,n(C),d(C)):console.log("Handlebars not found, init failed")}typeof Handlebars!="undefined"&&h2(Handlebars);})();
