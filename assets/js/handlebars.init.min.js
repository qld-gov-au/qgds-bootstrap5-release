"use strict";(()=>{function l(C){C.registerHelper("contains",function(t,i,a){return t=C.escapeExpression(t),i=C.escapeExpression(i),i.indexOf(t)>-1?a.fn(this):a.inverse(this)}),C.registerHelper("ifCond",function(t,i,a,e){switch(i){case"==":return t==a?e.fn(this):e.inverse(this);case"===":return t===a?e.fn(this):e.inverse(this);case"!=":return t!=a?e.fn(this):e.inverse(this);case"!==":return t!==a?e.fn(this):e.inverse(this);case"<":return t<a?e.fn(this):e.inverse(this);case"<=":return t<=a?e.fn(this):e.inverse(this);case">":return t>a?e.fn(this):e.inverse(this);case">=":return t>=a?e.fn(this):e.inverse(this);case"&&":return t&&a?e.fn(this):e.inverse(this);case"||":return t||a?e.fn(this):e.inverse(this);case"contains":return typeof t=="string"&&typeof a=="string"?t.toLowerCase().indexOf(a.toLowerCase())>=0?e.fn(this):e.inverse(this):e.inverse(this);default:return e.inverse(this)}}),C.registerHelper("isType",function(t,i,a){return t===i?a.fn(this):a.inverse(this)}),C.registerHelper("ifAny",function(...t){let i=t.pop();return t.some(a=>!!a)?i.fn(this):i.inverse(this)}),C.registerHelper("now",function(){return new Date().toISOString()}),C.registerHelper("formatDate",function(t,i,a){let e;if(t&&(e=new Date(t)),isNaN(e)&&i&&(e=new Date(i)),isNaN(e))return"Invalid Date";var e2=["January","February","March","April","May","June","July","August","September","October","November","December"],t2=e.getDate(),s=e2[e.getMonth()],r=e.getFullYear();switch(a){case"YYYY":return`${r}`;case"MMMM YYYY":return`${s} ${r}`;default:return`${t2} ${s} ${r}`}}),C.registerHelper("formatDateOrToday",function(t,i){let a=t||new Date().toISOString();return C.helpers.formatDate(t,a,i)})}typeof Handlebars!="undefined"&&l(Handlebars);var n=`<!-- QGDS Component: Accordion -->
...
{{/unless }}`;var u=`<!-- QGDS Component: Callout -->
...
`;var y=`{{#>header_wrap }}
  ...
  {{#>header_primary siteHeader=siteHeader }}
    ...
    {{/unless}}
  {{/header_primary }}
{{/header_wrap }}`;var x=`    <div class="qld__header__pre-header">
        <div class="container-fluid">
...
`;var T=`    <span {{#if dropdown_enabled}} class="dropdown dropdownCTA" {{/if}}>
                            <a {{#if dropdown_enabled}} id="dropdown{{id}}" role="button" data-bs-toggle="dropdown" aria-expanded="false" class="qld__header__cta-link" href="#" {{else}} href="{{url.value}}" class="qld__header__cta-link" {{/if}}>
                                <span class="qld__header__cta-link-text">{{{text.value}}}</span>
                                {{#if dropdown_enabled}}
                                    <svg aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg" class="qld__header__cta-link-icon">
        ...
        {{/if}}
                        </span>`;var S=`<header class="qld__header" role="banner">
...
<!-- HEADER END -->`;var R=`<div class="alert {{variantClass}} {{customClass}}" role="alert">
    {{#if heading}}
...
</div>`;var Q=`<!-- QGDS Component: Select -->
...
`;var B=`<div class="qld-side-navigation">
...
`;function o(C){C.registerPartial("accordion",n),C.registerPartial("banner",d),C.registerPartial("blockquote",c),C.registerPartial("breadcrumbs",p),C.registerPartial("breadcrumbsWrapper",f),C.registerPartial("button",h),C.registerPartial("callout",u),C.registerPartial("card",L),C.registerPartial("contentFooter",v),C.registerPartial("contentFooterWrapper",m),C.registerPartial("contentWrapper",g),C.registerPartial("correctincorrect",V),C.registerPartial("dateinput",H),C.registerPartial("footer",M),C.registerPartial("footerForgov",b),C.registerPartial("formcheck",Z),C.registerPartial("fullPage",_),C.registerPartial("globalAlert",w),C.registerPartial("head",q),C.registerPartial("header",y),C.registerPartial("header_preMenu",x),C.registerPartial("header_primary",k),C.registerPartial("header_sitePreHeader_CTA_menuItem",T),C.registerPartial("header_wrap",S),C.registerPartial("inpageAlert",R),C.registerPartial("inpagenav",D),C.registerPartial("mainContainerWrapper",O),C.registerPartial("metaDcTerms",A),C.registerPartial("metaOpenGraph",P),C.registerPartial("modal",E),C.registerPartial("navbar",I),C.registerPartial("pagination",N),C.registerPartial("quickexit",F),C.registerPartial("searchInput",G),C.registerPartial("select",Q),C.registerPartial("sidenav",B),C.registerPartial("sidenavWrapper",J),C.registerPartial("spinner",W),C.registerPartial("table",Y),C.registerPartial("tag",j),C.registerPartial("textarea",U),C.registerPartial("textbox",$),C.registerPartial("typography",z),C.registerPartial("video",X)}typeof Handlebars!="undefined"&&o(Handlebars);var K=!1;function C2(C){typeof C=="undefined"&&console.error("Handlebars.init requires HandleBars"),typeof C!="undefined"?(K?console.log("HandleBars Helpers And Partials already loaded, loading again"):K=!0,l(C),o(C)):console.log("Handlebars not found, init failed")}typeof Handlebars!="undefined"&&C2(Handlebars);})();
